...
function AnimalStatusBadge({ status }: { status: string }) {
  if (status === "GESTANTE")
    return <Badge className="bg-blue-100 text-blue-700 border-blue-200">{statusAnimalLabel[status as StatusAnimal]}</Badge>
  if (status === "LACTANDO")
    return <Badge className="bg-teal-100 text-teal-700 border-teal-200">{statusAnimalLabel[status as StatusAnimal]}</Badge>
  if (status === "VAZIA")
    return <Badge variant="outline">{statusAnimalLabel[status as StatusAnimal]}</Badge>
  if (status === "EM_PROTOCOLO")
    return <Badge className="bg-purple-100 text-purple-700 border-purple-200">{statusAnimalLabel[status as StatusAnimal]}</Badge>
  if (status === "INSEMINADA")
    return <Badge className="bg-amber-100 text-amber-700 border-amber-200">{statusAnimalLabel[status as StatusAnimal]}</Badge>
  if (status === "DESCARTE")
    return <Badge variant="destructive">{statusAnimalLabel[status as StatusAnimal]}</Badge>
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <Badge className={loteStatusClass[lote.status] ?? ""}>
          {loteStatusLabel[lote.status] ?? lote.status}
        </Badge>
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        <Card className="border-l-4 border-l-green-500">
          <CardHeader>
            <CardTitle className="text-xs text-muted-foreground uppercase tracking-wide">Vacas</CardTitle>
          </CardHeader>
          <CardContent>
            <span className="text-3xl font-bold">{lote.totalVacas}</span>
          </CardContent>
        </Card>

        <Card className="border-l-4 border-l-blue-500">
          <CardHeader>
            <CardTitle className="text-xs text-muted-foreground uppercase tracking-wide">Gestantes</CardTitle>
          </CardHeader>
          <CardContent>
            <span className="text-3xl font-bold">{lote.gestantes}</span>
          </CardContent>
        </Card>

        <Card className="border-l-4 border-l-teal-500">
          <CardHeader>
            <CardTitle className="text-xs text-muted-foreground uppercase tracking-wide">Partos este mês</CardTitle>
          </CardHeader>
          <CardContent>
            <span className="text-3xl font-bold">{lote.paridasNoMes}</span>
          </CardContent>
        </Card>

        <Card className="border-l-4 border-l-amber-500">
          <CardHeader>
            <CardTitle className="text-xs text-muted-foreground uppercase tracking-wide">Bezerros vivos</CardTitle>
          </CardHeader>
          <CardContent>
            <span className="text-3xl font-bold">{lote.bezerrosVivos}</span>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <Card>
          <CardHeader>
            <CardTitle>Touro de Repasse</CardTitle>
          </CardHeader>
          <CardContent className="space-y-1">
            ...
            <p className="text-sm text-muted-foreground">
              Encerramento:{" "}
              {lote.dataFechamento
                ? format(lote.dataFechamento, "dd/MM/yyyy", { locale: ptBR })
                : "—"}
            </p>
          </CardContent>
        </Card>
