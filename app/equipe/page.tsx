import { veterinarios, encarregados } from "@/lib/mock/data"
import { Badge } from "@/components/ui/badge"
import { Card, CardHeader, CardTitle, CardContent } from "@/components/ui/card"

function CargoBadge({ cargo }: { cargo: string }) {
  if (cargo === "GERENTE")
    return <Badge className="bg-purple-100 text-purple-700 border-purple-200">Gerente</Badge>
  if (cargo === "ENCARREGADO")
    return <Badge className="bg-blue-100 text-blue-700 border-blue-200">Encarregado</Badge>
      ...
      <section className="space-y-4">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {veterinarios.map((v) => (
            <Card
              key={v.id}
              className={v.ativo ? "border-l-4 border-l-blue-500" : "border-l-4 border-l-gray-300"}
            >
              <CardHeader>
      ...
      <section className="space-y-4">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {encarregados.map((e) => (
            <Card
              key={e.id}
              className={e.ativo ? "border-l-4 border-l-green-500" : "border-l-4 border-l-gray-300"}
            >
              <CardHeader>
                <div className="flex items-start justify-between gap-2">
              ...
              <CardContent className="space-y-2">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1">
                    {e.lotes.map((lote) => (
                      <Badge key={lote} variant="outline" className="text-xs">
                        {lote}
                      </Badge>
                    ))}
                  </div>
